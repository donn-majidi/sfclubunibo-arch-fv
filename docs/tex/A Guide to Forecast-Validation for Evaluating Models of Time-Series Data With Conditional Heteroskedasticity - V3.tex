...
\begin{document}
	...
	Comparisons between volatility forecasts are based on the distance, or expected loss, to the true conditional variance. \textcite{PATTON2011246} focuses on the definition of the loss function since, if the ranking of two forecasts obtained from a so-called "imperfect" volatility proxy is the same as the ranking that would be obtained from the latent variable, which is actually unfeasible, then the loss function can be identified as "robust". A robust loss function allows for the comparison of the accuracy of a forecast, on average, even if the latent variable of interest is not observable. 
	
	However, a robust loss function does not automatically make the proxy choice irrelevant. Its role is to guarantee that the comparison between forecasts is not systematically distorted by the usage of an imperfect variable in place of the latent conditional variance. Nevertheless, the proxy still influences the noise of the evaluation, the precision of the loss differential estimations, and in the end, the following statistical inference. In empirical applications, the most used volatility proxies are squared returns and realized volatility. The former are simpler to compute and daily data are sufficient to provide availability, while the latter provides a more informative ex-post measure, when intraday data are available.
	
	Following \textcite{PATTON2011246}, a loss function $L(\cdot, \cdot)$ can be defined as robust if the expected ranking between the two forecasts is preserved when the latent conditional variance is substituted by its observable proxy. Among the most used loss functions, in this paper we choose to use only Mean Squared Error (MSE) and Quasi-Likelihood loss (QLIKE), since these are the only two proven to be robust in the sense discussed in the cited paper. 
	
	Formally, let $\hat{\sigma}_t^2$ be an ex-post observable proxy, and let $\hat{h}_{i,t}$ denote the conditional variance forecast produced by model $i$. 
	
	Then, Mean Squared Error is defined as: 
	\begin{align}
	...
	Practically, the elimination rule determines which model has to be removed every time the null hypothesis of equal predictive capacity is rejected. In the implementation proposed by \textcite{HANSENLUNDE_NASON2011}, this rule is based on standardized loss differentials. Intuitively, the model removed at every step is the one that mostly contributes to the rejection of the null hypothesis, hence the model with the worse relative performance in the current set. The test is repeated on the restricted set until the null hypothesis cannot be rejected anymore.
	...
	\geq 1-\alpha.
	\end{align}
	This property justifies the term \textit{confidence set}: MCS does not claim to always identify a unique winner, but is constructed so that the truly superior models are excluded only with a controlled probability. The procedure yields a MCS p-value for each model. These p-values sum up the sequence of elimination tests and allow to assess if a given model belongs to the MCS for different confidence levels. A model is included in the MCS at a $1-\alpha$ level if and only if its p-value is bigger than $\alpha$. As a result, a low MCS p-value indicates that the model is less likely to belong to the superior models set, while a higher p-value shows that data do not give enough evidence to remove it from the set. As in standard statistical tests, this value is not to be intended as the probability that the given model is the best.
